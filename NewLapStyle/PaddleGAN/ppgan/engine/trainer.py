...
class Trainer:
    ...
    def __init__(self, cfg):
        ...
        if validate_cfg and 'metrics' in validate_cfg:
            # print("validate metrics cfg") # it does go in
        ...
        # evaluate only
        if not cfg.is_train:
            wandb.init(project="ml-lapstyle-2", entity="guti", config=cfg)
            return
    ...
    def test(self):
        ...
        if self.metrics:
            for metric in self.metrics.values():
                # print("metric reset") # It is being used
                metric.reset()
    ...
    def print_log(self):
        ...
        wandb.log({"Epoch": self.current_epoch, "learning rate": self.current_learning_rate, "current_iter": self.current_iter, "ips": self.ips, "Batch cost": self.step_time})
